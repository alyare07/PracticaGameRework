...
public final class CatalogoRecetas {
	...
	private static void inicializarRecetas() {
		// 1. Hacha de Tala Básica (A Mano / Manual: 5 Madera)
		registrarReceta(new RecetaCrafteo("hacha_tala_madera", "Hacha de Tala", EstacionCrafteo.MANUAL,
				new Ingrediente[] { new Ingrediente(ListaModelosItem.COD_RECURSO_MADERA, 5) },
				new Herramienta(ListaModelosItem.COD_HERRAMIENTA_HACHA, 8, 14, 350, TipoHerramienta.HACHA, 35.0)));

		// 2. Pico de Minería Básico (A Mano / Manual: 4 Madera + 4 Piedra)
		registrarReceta(new RecetaCrafteo("pico_mineria_piedra", "Pico de Minería", EstacionCrafteo.MANUAL,
				new Ingrediente[] { new Ingrediente(ListaModelosItem.COD_RECURSO_MADERA, 4),
						new Ingrediente(ListaModelosItem.COD_RECURSO_PIEDRA, 4) },
				new Herramienta(ListaModelosItem.COD_HERRAMIENTA_PICO, 6, 14, 400, TipoHerramienta.PICO, 30.0)));

		// 3. Poción de Vida Menor / Vendaje (A Mano / Fogata: 3 Madera + 2 Piedra)
		registrarReceta(new RecetaCrafteo("pocion_vida_menor", "Poción de Vida Menor", EstacionCrafteo.MANUAL,
				new Ingrediente[] { new Ingrediente(ListaModelosItem.COD_RECURSO_MADERA, 3),
						new Ingrediente(ListaModelosItem.COD_RECURSO_PIEDRA, 2) },
				new PocionVidaMenor(1)));

		// 4. Paquete de Munición 9mm (Requiere Mesa de Trabajo: 6 Piedra)
		registrarReceta(new RecetaCrafteo("caja_municion_9mm", "Caja Munición 9mm", EstacionCrafteo.MESA_TRABAJO,
				new Ingrediente[] { new Ingrediente(ListaModelosItem.COD_RECURSO_PIEDRA, 6) },
				CajaMunicion.crear9mm(0, 0, 15)));

		// 5. Granada T1 (Requiere Mesa de Trabajo: 8 Piedra)
		registrarReceta(new RecetaCrafteo("granada_t1", "Granada T1", EstacionCrafteo.MESA_TRABAJO,
				new Ingrediente[] { new Ingrediente(ListaModelosItem.COD_RECURSO_PIEDRA, 8) },
				new GranadaT1(1)));
	}
