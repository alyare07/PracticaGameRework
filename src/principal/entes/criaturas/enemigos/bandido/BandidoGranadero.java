...
public class BandidoGranadero extends Bandido{
    ...
    public BandidoGranadero(final double x, final double y, final double vida, final double vidaMaxima, final Mundo mundo) {
	...
	if (!this.ESTADO.containsKey(Estado.CAMINANDO)) {
	    this.ANIMACION.pintar(g, this.getPosicionXIntDibujado(), this.getPosicionYIntDibujado(), this.direccion, AnimacionesBandido.ESTANDAR, this.atrasDeComplemento, true);
	} else {
	    this.ANIMACION.pintar(g, this.getPosicionXIntDibujado(), this.getPosicionYIntDibujado(), this.direccion, AnimacionesBandido.CAMINANDO, this.atrasDeComplemento, true);
	}
    }
	...
	if (this.atacando) {
		...
		if (this.GT_ATAQUE_INICIAL_COOLDOWN.transcurrioMiliSegundos(this.getTiempoMsEsperaAtaqueInicial())) {
			...
			this.direccion = Constantes.FUNCIONES.getDireccionMirando(this.getPosicionXInt(), this.getPosicionYInt(), Constantes.JUGADOR.getPosicionXInt(),
