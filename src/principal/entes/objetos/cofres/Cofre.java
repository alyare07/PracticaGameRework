...
public abstract class Cofre extends Objeto{
	...
	this.NOMBRE = nombre;
	this.INVENTARIO = new InventarioVault(this, this.CANT_SLOT, this.CANT_MAX_SLOT_H, this.NOMBRE);
	...
	if (!Constantes.GLOBALES.viendoCofre && this.estado == EstadoCofre.CERRADO && Constantes.JUGADOR.getAreaInteraccionCofre().intersects(this.getArea())
		&& Constantes.TECLADO.TECLA_RECOGIENDO.presionado() && this.GT_COFRE_INTERACCION_VISIBILIDAD.transcurrioMiliSegundos(TIEMPO_MS_COFRE_INTERACCION_VISIBILIDAD)) {
	    this.estado = EstadoCofre.ABIERTO;
	    Constantes.GLOBALES.viendoCofre = true;
	    Constantes.GLOBALES.inventarioVault = this.INVENTARIO;
	    ...
	    this.cerrar();
	} else if (this.estado == EstadoCofre.ABIERTO && Constantes.TECLADO.TECLA_RECOGIENDO.presionado()
		&& this.GT_COFRE_INTERACCION_VISIBILIDAD.transcurrioMiliSegundos(TIEMPO_MS_COFRE_INTERACCION_VISIBILIDAD)) {
	    this.cerrar();
    ...
    @Override
    public String toString() {
	return "Cofre [X=" + this.x + ", Y=" + this.y + ", estado=" + this.estado + "]";
    }
