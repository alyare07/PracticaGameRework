...
public class ProyectilGranada extends ProyectilGeneral {
	...
	public ProyectilGranada(final int xDestino, final int yDestino, Mundo mundo, Ente causante, final Granada granada) {
		...
		this.AREA_DESTINO = new Ellipse2D.Double(xDestino-granada.getDiamentroAreaCaida()/2, yDestino-granada.getDiamentroAreaCaida()/2, granada.getDiamentroAreaCaida(), granada.getDiamentroAreaCaida());
		this.GRANADA = granada;
		this.ANIMACION_EXPLOSION = new Animacion(new HojaSprite(((ModeloGranada)ListaModelosItem.getModeloConsumible(this.GRANADA.getCodigoModelo())).getTexturaExplosion(), 50, false), false, this.TIEMPO_MS_FRAMES_EXPLOSION);
		this.DIRECCION_DESTINO = new Point();
		if(AREA_DESTINO.getCenterX() > x){
			this.DIRECCION_DESTINO.x = 1;
		}else if(AREA_DESTINO.getCenterX() < x) {
			this.DIRECCION_DESTINO.x = -1;
		}else {
			this.DIRECCION_DESTINO.x = 0;
		}
		if(AREA_DESTINO.getCenterY() > y){
			this.DIRECCION_DESTINO.y = 1;
		}else if(AREA_DESTINO.getCenterY() < y) {
			this.DIRECCION_DESTINO.y = -1;
		}else {
	...
	public ProyectilGranada(double damage, final Ellipse2D.Double areaDestino, Mundo mundo, final int xOrigen, final int yOrigen, final String codModelo) {
		super(damage, 1, false, 0, mundo, xOrigen, yOrigen, 10, 10, null, null);
	...
	public void pintar(final Graphics2D g) {
		if(this.realizoImpacto) {
			...
			DibujoDebug.dibujarFiguraEllipseRefCamara(g, AREA_DESTINO.getBounds(), Color.cyan);
			super.pintar(g);
		}
	...
	protected void verificarImpacto() {
		if(!this.realizoImpacto && this.x == this.AREA_DESTINO.getCenterX() && this.y == this.AREA_DESTINO.getCenterY()) {
			for(Criatura c : this.mundo.getCriaturasIntersectadas(AREA_DESTINO, !(this.CAUSANTE instanceof Jugador))) {
				this.impactar(c);
			}
	...
	protected void mover() {
		...
		if(this.causanteMasCercaAlPunto00) {
			...
			this.x = this.trayectoria[0][posTrayectoria];
			this.y = this.trayectoria[1][posTrayectoria];
			this.posTrayectoria++;
		}else {
	...
	private void generarTrayectoria() {
		...
		if(puntoOrigen.distance(punto00)< puntoDestino.distance(punto00)) {
		    ...
		    this.trayectoria = Constantes.FUNCIONES.GENERADOR_TRAYECTORIAS.getTrayectoiaBezier(puntoOrigen.x, puntoOrigen.y, puntoDestino.x, puntoDestino.y,this.GRANADA.getTiempoMsCaidaEnAnchoPantalla());
