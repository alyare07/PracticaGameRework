...
public class Inventario {
	...
	public Inventario() {
		...
		this.ANCHO = 202;
		this.ALTO = 104;
		this.X = Constantes.CENTROX - ANCHO / 2;
//		this.Y = Constantes.CENTROY - ALTO / 2;
		this.Y = Constantes.CENTROY;

		this.MARGEN_GENERAL = 2;
		this.ZONA_INFO_JUGADOR = new Rectangle(this.X, this.Y, this.ANCHO, 20);
		
		this.ZONA_SLOTS_EQUIPAMIENTOS= new Rectangle(this.X+22, this.Y+1, this.ANCHO-24, 18);
		
		
		this.ZONA_SLOTS_ALMACEN = new Rectangle(this.X, this.Y + 20, this.ANCHO, 62); // this.Y + 20;
		this.ZONA_SLOTS_PRINCIPALES = new Rectangle(this.X, this.Y + 82, this.ANCHO, 22);// this.Y + 82;
		this.AREA_PERSONAJE = new Rectangle(this.X + 2, this.Y + 2, 16, 16);
		
		
		this.SLOT_MANAGER = new SlotManager(this,MARGEN_GENERAL,ZONA_SLOTS_ALMACEN,ZONA_SLOTS_PRINCIPALES,ZONA_SLOTS_EQUIPAMIENTOS);
...
//		this.NEGRO_TRANSPARENTE = new Color(43, 24, 34, 200);
	...
	public void pintar(final Graphics2D g) {
		...
		pintarInventario(g);
		pintarItemSeleccionadoEnPuntero(g);
		this.pintarItemSeleccionadoEnPunteroInventarioTercero(g);
	}
	

	private void pintarInventario(final Graphics2D g) {
		DibujoDebug.dibujarRectanguloRelleno(g, ZONA_INFO_JUGADOR, GRIS_TRANSPARENTE);
		DibujoDebug.dibujarRectanguloRelleno(g, ZONA_SLOTS_ALMACEN, GRIS_TRANSPARENTE);
		DibujoDebug.dibujarRectanguloRelleno(g, ZONA_SLOTS_PRINCIPALES, NEGRO_TRANSPARENTE);
		DibujoDebug.dibujarRectanguloRelleno(g, AREA_PERSONAJE, BLANCO_TRANSPARENTE);
		DibujoDebug.dibujarRectanguloContorno(g, X, Y,ANCHO,ALTO,Color.lightGray);
		this.SLOT_MANAGER.pintar(g);
		Animaciones.JUGADOR.pintar(g,this.AREA_PERSONAJE.x, this.AREA_PERSONAJE.y);
	}
	...
	private void pintarItemSeleccionadoEnPunteroInventarioTercero(final Graphics2D g) {
		if(this.slotItemInventarioTerceroSeleccionado!=null && this.slotItemInventarioTerceroSeleccionado.contieneItem()) {
			DibujoDebug.dibujarImagen(g, this.slotItemInventarioTerceroSeleccionado.getItem().getTextura(), posicionPuntero);
		}
	}
	...
	public void actualizar(final Raton raton) {
		...
		this.SLOT_MANAGER.actualizar(raton, GE_RATON_PRESIONO, TIEMPO_ACTUALIZACION_RATON_PRESIONADO, posicionPuntero, this.mundo);
	...
	private void intercambioInventarioExterno(final Raton raton) {
		if(Constantes.GLOBALES.viendoCofre && raton.getRectanguloPosicionEscalado().intersects(Constantes.GLOBALES.inventarioVault.getArea())) {
			InventarioVault invExt = Constantes.GLOBALES.inventarioVault;
...
//					System.out.println("click dentro del area invExt2");
					...
					if(this.slotOrigenItemSeleccionado instanceof SlotArma) {
						if(!((slotApuntadoInvExt != null && slotApuntadoInvExt.contieneItem() && slotApuntadoInvExt.getItem() instanceof Arma)||(slotApuntadoInvExt != null && !slotApuntadoInvExt.contieneItem() ))) {
							this.deseleccionar();
...
//							System.out.println("CASO B1.1");
							Item aux = slotApuntadoInvExt.getItem();
