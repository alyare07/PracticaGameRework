...
public class ZonaTP extends Ente {
	...
	public void actualizar() {
		if(!this.ENTES_TELETRANSPORTADOS_HACIA_ACA.isEmpty()) {
			for(Criatura c : this.ENTES_TELETRANSPORTADOS_HACIA_ACA.values()) {
				if(c instanceof Jugador) {
					if(!((Jugador)c).getRectanguloInterseccionGeneral().intersects(this.AREA)) this.ENTES_TELETRANSPORTADOS_HACIA_ACA.remove(c);
				}
			}
		}
	}
	...
	public void pintar(final Graphics2D g) {
		DibujoDebug.dibujarRectanguloRellenoRefCamara(g, this.AREA, new Color(0.396f, 0.71764f, 0.835f, 0.43f));
	}
	...
	public boolean disponibleParaTP(final Criatura c) {
		if(this.estaHabilitado()) return !this.ENTES_TELETRANSPORTADOS_HACIA_ACA.containsKey(c);
		else return false;
	}
