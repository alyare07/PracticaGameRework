...
public abstract class Mapa{
	...
	for (final Object obj : listaMundoJson) {
	    ...
	    this.MUNDOS.put(nombreMundo, this.generarMundo((JSONObject) jsonMapa.get(nombreMundo), gc, porcentajeCarga));
	...
	if (esc == null) {
	    ...
	    JOptionPane.showConfirmDialog(null, "Error al cargar el mundo " + ruta.getPath(), "Error", JOptionPane.ERROR_MESSAGE);
	    System.exit(0);
	}
	...
	for (final Object obj : (JSONArray) jsonMundo.get(Constantes.FUNCIONES.GESTOR_TIPOS_EN_CARGA.getTipo(Spawn.class))) {
	    jsonSpawn = (JSONObject) obj;
	    listaSpawn.add(new Spawn(Integer.parseInt(jsonSpawn.get("x").toString()), Integer.parseInt(jsonSpawn.get("y").toString()), jsonSpawn.get("nombre").toString()));
	    if (jsonSpawn.get("nombre").toString().equals(Mundo.CLAVE_PUNTO_SPAWN_COMIENZO)) {
		comienzo = new Spawn(Integer.parseInt(jsonSpawn.get("x").toString()), Integer.parseInt(jsonSpawn.get("y").toString()), jsonSpawn.get("nombre").toString());
	    }
	}
	final Mundo m = new Mundo(new Escenario(new Terreno((JSONObject) jsonMundo.get(Constantes.FUNCIONES.GESTOR_TIPOS_EN_CARGA.getTipo(Tile.class))),
		...
		((JSONArray) jsonMundo.get(Constantes.FUNCIONES.GESTOR_TIPOS_EN_CARGA.getTipo(Objeto.class))).toString()), comienzo.getPoint());
