...
public class MapaCargadoDeTemp extends Mapa {
	...
	public MapaCargadoDeTemp(final GestorCarga gc, final int porcentajeCarga, final GestorPartida gp,
	...
	private Mundo generarMundo(final JSONObject jsonMundo) {
		return new Mundo(new Escenario(new Terreno(
				...
				((JSONArray) jsonMundo.get(Globales.FUNCIONES.GESTOR_TIPOS_EN_CARGA.getTipo(Objeto.class)))
						.toString()),
				new Point(Integer.parseInt(jsonMundo.get("Punto Comienzo X").toString()),
						Integer.parseInt(jsonMundo.get("Punto Comienzo Y").toString())));
	}
