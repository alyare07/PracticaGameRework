...
public class RenderEntidad {
	...
	public boolean estaRenderizado() {
		return this.codAct == mundo.getCodAct();
	}
...
//		this.codPaint = this.mundo.getCodAct();
	...
	private void verificarZoneBox() {
		...
		this.mundo.getZonasIntersectadas(ENTIDAD).forEach((zb) -> {
		...
		if (this.lista.size() == 0) {
			...
			System.out.println("Sin mapa, se ha eliminado entidad " + this.ENTIDAD + " / rest: "
					+ this.mundo.getRenders().getCantEntidades());
		}
	}
	...
	private void eliminarEntidad() {
		this.limpiarZonas();
		this.mundo.getRenders().eliminarEntidad(ENTIDAD);
	}
