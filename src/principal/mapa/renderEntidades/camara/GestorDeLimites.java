...
public class GestorDeLimites extends Ente {
	 ...
	 * sobrepasa alguno de los limites. De ser asi tomara el control de la/s 
	 * ejes (X/Y) que se haya sobrepasado para no desplazar mas la camara en
	 * dicho/s eje/s. Se devolvera el control cuando  la entidad deje de 
	 * sobrepasar los limites.
	 */
	public void actualizar() {
		if(this.entidadEnfocada == null ) return;
...
//				System.out.println("Limite maximo Y: "+this.limiteMaximoY);
	...
	public void setEntidadEnfocada(final Ente e) {
		...
		this.limiteMaximoX = entidadEnfocada.getMundo().getMapa().getAncho() - Constantes.ANCHO_JUEGO/2 - entidadEnfocada.getArea().width/2;
		this.limiteMaximoY = entidadEnfocada.getMundo().getMapa().getAlto() - Constantes.ALTO_JUEGO/2 - entidadEnfocada.getArea().height/2;
		this.limiteMinimoX = Constantes.ANCHO_JUEGO/2 - entidadEnfocada.getArea().width/2;
		this.limiteMinimoY = Constantes.ALTO_JUEGO/2 - entidadEnfocada.getArea().height/2;
	}
	
	public void setEntidadEnfocada(final Ente e, final int limiteMaximoX, final int limiteMinimoX, final int limiteMaximoY, final int limiteMinimoY, final boolean contarDimensionEnte) {
		this.entidadEnfocada = e;
		this.limiteMaximoX = contarDimensionEnte? limiteMaximoX - entidadEnfocada.getArea().width/2 : limiteMaximoX;
		this.limiteMaximoY = contarDimensionEnte? limiteMaximoY - entidadEnfocada.getArea().height/2 : limiteMaximoY;
		this.limiteMinimoX = contarDimensionEnte? limiteMinimoX - entidadEnfocada.getArea().width/2 : limiteMinimoX;
		this.limiteMinimoY = contarDimensionEnte? limiteMinimoY - entidadEnfocada.getArea().height/2 : limiteMinimoY;
	}
	...
	public int getPosicionXInt() {
		if(this.gestionandoX) return this.x;
		else return this.entidadEnfocada.getPosicionXInt();
	}

	@Override
	public int getPosicionYInt() {
		if(this.gestionandoY) return this.y;
		else return this.entidadEnfocada.getPosicionYInt();
	}

	@Override
	public double getPosicionX() {
		if(this.gestionandoX) return this.x;
		else return this.entidadEnfocada.getPosicionX();
	}

	@Override
	public double getPosicionY() {
		if(this.gestionandoY) return this.y;
		else return this.entidadEnfocada.getPosicionY();
	}
