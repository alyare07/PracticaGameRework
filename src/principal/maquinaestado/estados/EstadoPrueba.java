...
public class EstadoPrueba implements EstadoJuego{
    ...
    public EstadoPrueba() {
	Constantes.CAMARA.setEntidadEnfocada(this.JUGADOR);
	this.MUNDO = new Mundo(new Escenario(new Terreno(100, 100, 16, ListaModeloTile.COD_CESPED_3), "[]", "[]", "[]", "[]"), this.PUNTO_COMIENZO);
	this.JUGADOR.restablecerYCambiarMundo(this.MUNDO);
	this.INVENTARIO.establecerMundo(this.MUNDO);
	...
	if (this.TECLADO.TECLA_ESCAPE.presionado()) {
	    System.exit(0);
	}
	...
	if (Constantes.GLOBALES.viendoCofre) {
	    Constantes.GLOBALES.inventarioVault.actualizar(this.RATON, this.MUNDO, Constantes.GLOBALES.viendoCofre);
	}
	{
	    final Shape s = Constantes.JUGADOR.getAreaInterseccionMovimiento();
	    this.tilePisado = this.MUNDO.getTerreno().getTileReferenciado(s.getBounds().x + s.getBounds().width / 2, s.getBounds().y + s.getBounds().height);
	...
	if (this.AREA_TITULO.width == 0) {
	    this.AREA_TITULO.height = Constantes.FUNCIONES.MEDIDOR_STRING.medirAltoPixeles(g, this.TITULO);
	    this.AREA_TITULO.width = Constantes.FUNCIONES.MEDIDOR_STRING.medirAnchoPixeles(g, this.TITULO);
	    this.AREA_TITULO.x = Constantes.ANCHO_JUEGO / 2 - this.AREA_TITULO.width / 2;
	    this.AREA_TITULO.y = this.AREA_TITULO.height / 2 + 10;
	}
	DibujoDebug.dibujarRectanguloRelleno(g, this.AREA_TITULO.x, this.AREA_TITULO.y, this.AREA_TITULO.width, this.AREA_TITULO.height + 2, Color.black);
	DibujoDebug.dibujarString(g, this.TITULO, this.AREA_TITULO.x, this.AREA_TITULO.y + this.AREA_TITULO.height, Color.white);
    }
	...
	if (Constantes.TECLADO.TECLA_DEBUG_TILE_INFO.presionado()) {
	    DibujoDebug.dibujarString(g, this.tilePisado != null ? this.tilePisado.toString() : "PuntoTile: (none)", 120, 20, Color.white);
	    if (this.tilePisado != null) {
	...
	if (Constantes.TECLADO.TECLA_DEBUG.presionado()) {
	    ...
	    DibujoDebug.dibujarString(g, "Estados: " + this.JUGADOR.getStringEstados(), 20, 305);
	}
    }
