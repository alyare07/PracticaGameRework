...
public class GrupoPaleta {
	...
	private void iniciarPaletas() {
		...
		final PaletaComplento paletaRecursos = new PaletaComplento(this.AREA.x, yPaleta, this.AREA.width, altoPaleta,
				32);
		paletaRecursos.agregarEntrada("Árbol Talable 1", Textura.getTextura(Textura.TEXTURA_x32_ARBOL_1), true,
				(x, y) -> new ArbolCosechable(x, y, Textura.TEXTURA_x32_ARBOL_1));
		paletaRecursos.agregarEntrada("Árbol Talable 2", Textura.getTextura(Textura.TEXTURA_x32_ARBOL_2), true,
				(x, y) -> new ArbolCosechable(x, y, Textura.TEXTURA_x32_ARBOL_2));
		paletaRecursos.agregarEntrada("Árbol Nevado Talable", Textura.getTextura(Textura.TEXTURA_x32_ARBOL_1_NEVADO),
				true, (x, y) -> new ArbolCosechable(x, y, Textura.TEXTURA_x32_ARBOL_1_NEVADO));
		paletaRecursos.agregarEntrada("Roca Minable", Textura.getTextura(Textura.TEXTURA_x16_MURO_PIEDRA_NEGRA), true,
				(x, y) -> new RocaCosechable(x, y, Textura.TEXTURA_x16_MURO_PIEDRA_NEGRA));
		...
		final PaletaComplento paletaObjetos = new PaletaComplento(this.AREA.x, yPaleta, this.AREA.width, altoPaleta,
				32);
		paletaObjetos.agregarEntrada("Casa Grande", Textura.getTextura(Textura.TEXTURA_X64_CASA1), false,
				(x, y) -> new Complemento(x, y, ListaModeloComplemento.COD_CASA_1));
		paletaObjetos.agregarEntrada("Árbol Decorativo 1", Textura.getTextura(Textura.TEXTURA_x32_ARBOL_1), false,
				(x, y) -> new Complemento(x, y, ListaModeloComplemento.COD_ARBOL_1));
		paletaObjetos.agregarEntrada("Árbol Decorativo 2", Textura.getTextura(Textura.TEXTURA_x32_ARBOL_2), false,
				(x, y) -> new Complemento(x, y, ListaModeloComplemento.COD_ARBOL_2));
		paletaObjetos.agregarEntrada("Cofre Mediano", Textura.getTextura(Textura.TEXTURA_x16_ANILLO_ORO), false,
				(x, y) -> new CofreMediano(x, y));
		paletaObjetos.agregarEntrada("Cofre Pequeño", Textura.getTextura(Textura.TEXTURA_x16_ANILLO_PLATA), false,
				(x, y) -> new CofrePequeño(x, y));
		paletaObjetos.agregarEntrada("Árbol Cofre Secreto", Textura.getTextura(Textura.TEXTURA_x32_ARBOL_2), false,
				(x, y) -> new ArbolCofre(x, y));
		paletaObjetos.agregarEntrada("Barrera Invisible", Textura.getTextura(Textura.TEXTURA_TRANSPARENTE), false,
				(x, y) -> new Complemento(x, y, ListaModeloComplemento.COD_BARRERA_INVISIBLE));
		paletaObjetos.agregarEntrada("Cuadrado Invisible", Textura.getTextura(Textura.TEXTURA_TRANSPARENTE), false,
