...
//	protected final Rectangle AREA_VISTA;
	...
	public MenuConfiguracion(GestorEstados ge) {
		super(ge);
...
//		this.AREA_VISTA =  new Rectangle(this.COMIENZO_X, this.COMIENZO_Y, this.ANCHO_AREA, 300);
	...
	public void actualizar() {
		...
		for (Componente c : this.COMPONENTES_DESPLAZABLES) {
			if (c.visible()) {
				c.actualizar();
				if(c instanceof CajaTecla) {
					((CajaTecla)c).establecerDesplazamientoY(desplazamiento);
				}
			}
		}
...
//		DibujoDebug.dibujarRectanguloRelleno(g, new Rectangle(this.AREA_CONFIGURACIONES.x,this.AREA_CONFIGURACIONES.y-desplazamiento,this.AREA_CONFIGURACIONES.width,this.AREA_CONFIGURACIONES.height), Color.orange);
		DibujoDebug.dibujarRectanguloContorno(g, new Rectangle(this.AREA_CONFIGURACIONES.x,this.AREA_CONFIGURACIONES.y-desplazamiento-1,this.AREA_CONFIGURACIONES.width,this.AREA_CONFIGURACIONES.height), Color.red);
		for (Componente c : this.COMPONENTES) {
			if (c.visible()) {
				c.pintar(g);
			}
	...
	protected void actualizarVisibilidadBotonesSubirBajar() {
		...
		}else if(this.AREA_CONFIGURACIONES.y+this.AREA_CONFIGURACIONES.height > this.DIMENSION.height && this.desplazamiento == 0){
			if(this.btnSubir.visible()) {
				this.btnSubir.visible(false);
			}
			if(!this.btnBajar.visible()) {
		...
		}else if(this.AREA_CONFIGURACIONES.y+this.AREA_CONFIGURACIONES.height > this.DIMENSION.height && (this.DIMENSION.height+this.desplazamiento>=this.AREA_CONFIGURACIONES.y+this.AREA_CONFIGURACIONES.height)){
			if(!this.btnSubir.visible()) {
	...
	//HAY QUE HACER QUE LOS BOTONES SUBIR Y BAJAR SOLO SE VEAN CUANDO CORRESPONDAN Y NO EN TODO MOMENTO!!!!
	...
	protected void inicializarBotones() {
		...
		final Boton subir = new Boton("Subir", Color.gray, new Rectangle(this.COMIENZO_X+anchoArea+4, COMIENZO_Y, 20, 15));
		subir.establecerAccion( () -> {
			moverArriba();
		});
		
		final Boton bajar = new Boton("Bajar", Color.gray, new Rectangle(this.COMIENZO_X+anchoArea+4, this.DIMENSION.height-this.COMIENZO_Y, 20, 15));
		bajar.establecerAccion( () -> {
			moverAbajo();
		});
		
		final Boton volver = new Boton("Volver", Color.gray, new Rectangle(2, this.DIMENSION.height-this.COMIENZO_Y, 24, 15));
		...
		final Boton guardar = new Boton("Guardar", Color.gray, new Rectangle(2, this.DIMENSION.height-this.COMIENZO_Y-20, 24, 15));
		...
		guardar.establecerAccion(() ->{
			...
			Constantes.TECLADO.guardarConfig();
		});
	...
	protected void inicializarComponentesDesplazables() {
		...
		final CajaTecla ctInventario = new CajaTecla(new Rectangle(lbInventario.getPunto().x + lbInventario.getAncho()+margenXLabelCT , lbInventario.getPunto().y-lbInventario.getAlto()+margenXLabelCT,80,10), Color.white, Color.black,Constantes.TECLADO.TECLA_INVENTARIO);
...
//		final CajaTecla ctCorrer = new CajaTecla(new Rectangle(lbCorrer.getPunto().x + lbCorrer.getAncho()+margenXLabelCT , lbCorrer.getPunto().y-lbCorrer.getAlto()+margenXLabelCT,80,10), Color.white, Color.black,Constantes.TECLADO.TECLA_CORRIENDO);
		...
		final CajaTecla ctCorrer = this.generarCajaTeclaPosicionado(lbCorrer, Color.white, Color.black, Constantes.TECLADO.TECLA_CORRIENDO, margenXLabelCT);
		
		final Label lbAtacar = this.generarLabelPosicionado("Atacar", lbCorrer, Color.white, 9f);
		final CajaTecla ctAtacar = this.generarCajaTeclaPosicionado(lbAtacar, Color.white, Color.black, Constantes.TECLADO.TECLA_ATACANDO, margenXLabelCT);
		
		final Label lbRecoger = this.generarLabelPosicionado("Recoger", lbAtacar, Color.white, 9f);
		final CajaTecla ctRecoger = this.generarCajaTeclaPosicionado(lbRecoger, Color.white, Color.black, Constantes.TECLADO.TECLA_RECOGIENDO, margenXLabelCT);
		
		final Label lbMoverArriba = this.generarLabelPosicionado("Mover Arriba", lbRecoger, Color.white, 9f);
		final CajaTecla ctMoverArriba = this.generarCajaTeclaPosicionado(lbMoverArriba, Color.white, Color.black, Constantes.TECLADO.TECLA_ARRIBA, margenXLabelCT);
		
		final Label lbMoverAbajo = this.generarLabelPosicionado("Mover Abajo", lbMoverArriba, Color.white, 9f);
		final CajaTecla ctMoverAbajo = this.generarCajaTeclaPosicionado(lbMoverAbajo, Color.white, Color.black, Constantes.TECLADO.TECLA_ABAJO, margenXLabelCT);
		
		final Label lbMoverIzquierda = this.generarLabelPosicionado("Mover Izquierda", lbMoverAbajo, Color.white, 9f);
		final CajaTecla ctMoverIzquierda = this.generarCajaTeclaPosicionado(lbMoverIzquierda, Color.white, Color.black, Constantes.TECLADO.TECLA_IZQUIERDA, margenXLabelCT);
		
		final Label lbMoverDerecha = this.generarLabelPosicionado("Mover Derecha", lbMoverIzquierda, Color.white, 9f);
		final CajaTecla ctMoverDerecha = this.generarCajaTeclaPosicionado(lbMoverDerecha, Color.white, Color.black, Constantes.TECLADO.TECLA_DERECHA, margenXLabelCT);
...
//		this.ultimoLabelLineaInferior(lbMoverDerecha);
	...
	/***
	 * Permite determinar la visibilidad del boton guardar en base a si las cajas de teclas possen valores diferentes a los determinado en sus Teclas referenciadas
	 * 
	 * **/
	...
	/**
	 * Permite generar un label en base al label posicionado en la linea superior al que se generara este, de forma que este label quede en la linea de abajo
	 * 
	 * @param texto -> es el texto que mostrara el label
	 * @param labelSuperior -> es el label que se usa como referencia para generar este label pero en la siguiente linea de abajo
	 * @param colorLetra -> es el color que tendra la letra
	 * @param tamanoLetra -> es el tamano que tendran las letras del texto en el label
	 * @return Label -> retorna un nuevo label en base a los parametros pasados
	 * 
	 * **/
	private Label generarLabelPosicionado(final String texto,final Label labelSuperior,final Color colorLetra, final float tamanoLetra) {
		return new Label(texto, labelSuperior.getPunto().x ,labelSuperior.getPunto().y+20, colorLetra, tamanoLetra);
	}
	
	/**
	 * Permite generar una CajaTecla en base al label posicionado en el costado izquierdo al que se generara este, de forma que esta CajaTecla quede en al lado derecho del label
	 * 
	 * @param labelIzquierdo -> es el label que se usa como referencia para generar esta CajaTecla pero al costado derecho del label
	 * @param colorFondo -> es el color que tendra el fondo de la CajaTecla
	 * @param colorLetra -> es el color que tendra la letra
	 * @param teclaReferencia -> es la tecla a la que se cambiaran los valores en el teclado y usaran como referencia 
	 * @return CajaTecla -> retorna una CajaTecla en base a los parametros pasados
	 * 
	 * **/
	private CajaTecla generarCajaTeclaPosicionado(final Label labelIzquierdo, final Color colorFondo, final Color colorLetra, final Tecla teclaReferencia, final int margenXLabelCT) {
		return new CajaTecla(new Rectangle(labelIzquierdo.getPunto().x + labelIzquierdo.getAncho()+margenXLabelCT , labelIzquierdo.getPunto().y-labelIzquierdo.getAlto()+margenXLabelCT,80,10), colorFondo, colorLetra, teclaReferencia);
	}
	...
	private void ultimoLabelLineaInferior(final Label l) {
		this.AREA_CONFIGURACIONES.height = l.getPunto().y+l.getAlto() -COMIENZO_Y + 4;
	}
