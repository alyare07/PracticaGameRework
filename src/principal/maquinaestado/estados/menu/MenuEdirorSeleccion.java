...
public class MenuEdirorSeleccion extends Menu {
	...
	public MenuEdirorSeleccion(GestorEstados ge) {
		super(ge);
	}
	...
	protected void inicializarBotones() {
		...
		final Boton mapaExistente = new Boton("Abrir Mapa", Color.gray, new Rectangle(xBoton, 160, anchoBoton, altoBoton));
	...
	private void pintarAvisoCarga(final Graphics2D g) {
		...
		final int ancho = Constantes.FUNCIONES.MEDIDOR_STRING.medirAnchoPixeles(g, textoCarga);
		final int alto = Constantes.FUNCIONES.MEDIDOR_STRING.medirAltoPixeles(g, textoCarga);
		final int puntoX = (this.DIMENSION.width / 2) - (ancho / 2);
		final int puntoY = (this.DIMENSION.height / 2) + (alto / 2);
		DibujoDebug.dibujarString(g, textoCarga, puntoX, puntoY, Color.YELLOW);
		g.setFont(g.getFont().deriveFont(Constantes.TAMANO_FUENTE));

	}

	private void abrirMapa() {
		JFileChooser selector = new JFileChooser(new File("./mapas/"));
		...
		if (seleccion == JFileChooser.APPROVE_OPTION) {
			...
			Thread hilo = new Thread(() -> {
				this.GE.editorMapa(EscenarioLoader.importarEscenario(new File(selector.getSelectedFile().getAbsolutePath())).getMapa());
			});
			hilo.start();
