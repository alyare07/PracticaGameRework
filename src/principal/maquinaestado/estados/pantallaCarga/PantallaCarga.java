...
public class PantallaCarga implements EstadoJuego {
	...
	protected final AsistenteCamara AC = new AsistenteCamara((this.DIMENSION.width / 2) - 1,
			(this.DIMENSION.height / 2) - 1, 2, 2);

	public PantallaCarga(final GestorCarga gc) {
		this.FONDO = Constantes.FUNCIONES.TEXTURAS_TOOLS.crearTextura(new Color(61, 81, 61), this.DIMENSION.width,
	...
	public PantallaCarga(final GestorCarga gc, final Color fondo) {
		this.FONDO = Constantes.FUNCIONES.TEXTURAS_TOOLS.crearTextura(fondo, this.DIMENSION.width,
				this.DIMENSION.height);
		this.GC = gc;
	}

	public PantallaCarga(final GestorCarga gc, final String ruta) {
		this.FONDO = Constantes.FUNCIONES.TEXTURAS_TOOLS.crearTextura(Color.cyan, 1, 1);
		this.GC = gc;
		this.mundo = new Mundo(EscenarioLoader.importarEscenario(new File(ruta)),
				new Point(this.DIMENSION.width / 2, this.DIMENSION.height / 2));
		Constantes.CAMARA.setEntidadEnfocada(this.AC);
	}
	...
	public void disposeMundo() {
		if (this.mundo != null) {
			this.mundo.getDijkstra().destruir();
			this.mundo = null;
		}
	}
