...
public class HojaSprite {
	...
	public HojaSprite(final String ruta, final int ladoSprites, final boolean hojaOpaca) {
		...
		if (hojaOpaca) {
			imagen = Globales.FUNCIONES.CARGADOR_RECURSOS.cargarImagenCompatibleOpaca(ruta);
		} else {
			imagen = Globales.FUNCIONES.CARGADOR_RECURSOS.cargarImagenCompatibleTranslucida(ruta);
		}
	...
	public HojaSprite(final BufferedImage imagen, final int anchoSprites, final int altoSprites,
	...
	private void rellenarSpritesDesdeImagen(final BufferedImage imagen) {
		for (int y = 0; y < this.altoHojaEnSprites; y++) {
			for (int x = 0; x < this.anchoHojaEnSprites; x++) {
				...
				this.sprites[x + (y * this.anchoHojaEnSprites)] = imagen.getSubimage(posicionX, posicionY,
						this.anchoSprites, this.altoSprites);
			}
		}
	}
	...
	public static BufferedImage getSpriteEspecifico(final String ruta, final int x, final int y, final int ancho,
		...
		if (opaca) {
			return Globales.FUNCIONES.CARGADOR_RECURSOS.cargarImagenCompatibleOpaca(ruta).getSubimage(x, y,
					ancho, alto);
		}
		return Globales.FUNCIONES.CARGADOR_RECURSOS.cargarImagenCompatibleTranslucida(ruta).getSubimage(x, y,
				ancho, alto);
	...
	public BufferedImage getSpriteInvertidoHorizontal(final int indice) {
		return Globales.FUNCIONES.TEXTURAS_TOOLS.voltearImagenH(this.sprites[indice]);
	}
	...
	public BufferedImage getSpriteInvertidoVertical(final int indice) {
		return Globales.FUNCIONES.TEXTURAS_TOOLS.voltearImagenV(this.sprites[indice]);
	}
	...
	public BufferedImage getSpriteRotado90GradosIzquierda(final int indice) {
		return Globales.FUNCIONES.TEXTURAS_TOOLS.voltearImagen90GradosIzquierda(this.sprites[indice]);
	}
	...
	public BufferedImage getSpriteRotado90GradosDerecha(final int indice) {
		return Globales.FUNCIONES.TEXTURAS_TOOLS.voltearImagen90GradosDerecha(this.sprites[indice]);
	}
