...
/**
 * Encargada de obtener todos los sprites 
 * de una imagen y gestionarlos segun se
...
public class HojaSprite {
	...
	public HojaSprite(final String ruta, final int ladoSprites, final boolean hojaOpaca) {
		...
		sprites = new BufferedImage[anchoHojaEnSprites * altoHojaEnSprites];
	...
	/**
	 * Rellena el vector de sprites obteniendolos desde la imagen mencionada
	 * los atributos que tambien se usaran son: anchoHojaEnPixeles, altoHojaEnPixeles,
	...
	private void rellenarSpritesDesdeImagen(final BufferedImage imagen) {
		for (int y = 0; y < altoHojaEnSprites; y++) {
			for (int x = 0; x < anchoHojaEnSprites; x++) {
				...
				this.sprites[x + y * anchoHojaEnSprites] = imagen.getSubimage(posicionX, posicionY, anchoSprites,
						altoSprites);
			}
		}
	}
	 ...
	 */
	public static BufferedImage getSpriteEspecifico(final BufferedImage imagen, final int x, final int y,
	 ...
	 * Obtener un sprite de una imagen.
	 * @param imagen La imagen de la que se sacara el sprite.
	 * @param indice La posicion del sprite a obtener de la imagen (indice avanza de 1 en 1, izq a der, arriba hacia abajo)
	 * @param lado Especifica el valor del ancho y alto que tendra cada sprite en dicha imagen.
	...
	/**
	 * 
	 * @param ruta La ruta donde se encuentra la imagen a cargar para sacar el sprite.
	 * @param x La posicion X del sprite en la imagen.
	 * @param y La posicion Y del sprite en la imagen.
	 * @param ancho El ancho del sprite en la imagen.
	 * @param alto El alto del sprite en la imagen.
	 ...
	 */
	public static BufferedImage getSpriteEspecifico(final String ruta, final int x, final int y, final int ancho,
			final int alto, final boolean opaca) {
		if (opaca) {
	 ...
	 * 
	 * @param ruta La ruta donde se encuentra la imagen a cargar para sacar el sprite.
	 * @param indice indice La posicion del sprite a obtener de la imagen (indice avanza de 1 en 1, izq a der, arriba hacia abajo)
	 * @param lado Especifica el valor del ancho y alto que tendra cada sprite en dicha imagen.
	 * @param opaca (True) -> Opaca. (False) -> Translucida
	 * @return El sprite obtenido de la posicion mencionada para la imagen cargada.
	 */
	public static BufferedImage getSpriteIndice(final String ruta, final int indice, final int lado,
			final boolean opaca) {
	...
	public BufferedImage getSpriteInvertidoHorizontal(final int indice) {
		return Textura.voltearImagenH(sprites[indice]);
	}
	/**
	...
	public BufferedImage getSpriteInvertidoVertical(final int indice) {
		return Textura.voltearImagenV(sprites[indice]);
	}
	/**
	...
	public BufferedImage getSpriteRotado90GradosIzquierda(final int indice) {
		return Textura.voltearImagen90GradosIzquierda(sprites[indice]);
	}
	/**
	...
	public BufferedImage getSpriteRotado90GradosDerecha(final int indice) {
		return Textura.voltearImagen90GradosDerecha(sprites[indice]);
	}
